<template>
  <footer class="footer footer--06">
    <div class="footer__main_box--06">
      <div class="container footer__container">
        <div class="footer__left_box--06">
          <a class="footer__logo_box footer__logo_box--06">
            <img class="footer__logo footer__logo--06" src="../assets/images/logo.png">
          </a>
          <span class="footer__description footer__description--06">Itmeo is a software and design company. Our team creates useful tools for web designers and developers.</span>
          <ul class="social">
            <li class="social__item social__item--left_margin"><a class="social__link social__link--twitter--black"></a></li>
            <li class="social__item social__item--left_margin"><a class="social__link social__link--facebook--black"></a></li>
            <li class="social__item social__item--left_margin"><a class="social__link social__link--behance--black"></a></li>
          </ul>
        </div>
        <div class="footer__right_box--06">
          <ul class="footer__menu footer__menu--06">
            <li class="footer__menu__item footer__menu__footer">
              <span class="footer__menu__title">other projects</span>
            </li>
            <li class="footer__menu__item">
              <a class="footer__menu__link">Phoenix Startup</a>
            </li>
            <li class="footer__menu__item">
              <a class="footer__menu__link">WebGradients</a>
            </li>
            <li class="footer__menu__item">
              <a class="footer__menu__link">UI Market</a>
            </li>
          </ul>
          <ul class="footer__menu footer__menu--06">
            <li class="footer__menu__item footer__menu__footer">
              <span class="footer__menu__title">get in touch</span>
            </li>
            <li class="footer__menu__item">
              <a class="footer__menu__link">About</a>
            </li>
            <li class="footer__menu__item">
              <a class="footer__menu__link">Contact Us</a>
            </li>
            <li class="footer__menu__item">
              <a class="footer__menu__link">Advertising</a>
            </li>
          </ul>
          <ul class="footer__menu footer__menu--06">
            <li class="footer__menu__item footer__menu__footer">
              <span class="footer__menu__title">follow the best</span>
            </li>
            <li class="footer__menu__item">
              <form class="footer__form">
                <input class="footer__input" type="email" placeholder="Your email" name="EMAIL" required>
                <button class="footer__button" type="submit"></button>
                <div class="footer__message footer__message--red">An email address must contain a single @</div>
                <div class="footer__message footer__message--green">Thanks!<br>Please click the link in the email we've just sent you.</div>
              </form>
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div class="footer__description_box--06">
      <div class="container footer__container">
        <span class="footer__copyright footer__copyright--06">© itmeo.com, 2017</span>
      </div>
    </div>
  </footer>
</template>
<style lang="stylus">
.footer
  background-color #fff
.footer__container
  position relative
@media (max-width: 992px)
  .footer__social_box
    position absolute
    top: 0
    right: 15px
.footer__main_box--06
  padding 49px 0 75px
  border-bottom 1px solid rgba(188,198,208,0.4)
  &:after
    content: ''
    visibility hidden
    display block
    height: 0
    clear both
@media (max-width: 500px)
  .footer__main_box--06
    padding-bottom: 35px
@media (min-width: 993px)
  .footer__left_box--06
    float:left

@media (max-width: 992px)
  .footer__left_box--06
    margin-bottom:40px

.footer__logo_box
  display inline-block
  width: 82px
  transition opacity .2s ease
  img
    width: 100%
  &:hover
    opacity .8
@media (max-width: 992px)
  .footer__logo_box
    margin-bottom: 20px
.footer__logo_box--06
  display: block
  margin-bottom: 28px
.footer__description
  font-size 13px
  line-height 1.6
  letter-spacing 0.05em
.footer__description--06
  display inline-block
  max-width 300px
  margin-bottom: 60px
@media (max-width: 992px)
  .footer__description--06
    margin-bottom:20px

.social
.footer__menu
  padding: 0
  margin: 0
  list-style none
.social__item
 display inline-block
.social__item--left_margin
  margin-left: 15px
  display inline-block
  &:first-child
    margin-left: 0
.social__link
  display inline-block
  position relative
  vertical-align middle
  transition opacity .2s ease
  &:hover
    opacity .8
  &:before
    content ""
    position absolute
    left: 0
    right: 0
    bottom: 0
    top: 0
.social__link--twitter--black
  width: 14px
  height: 12px
  &:before
   background: url('data:image/svg+xml;base64,') no-repeat center/14px 12px
.social__link--facebook--black
  width: 6px
  height: 12px
  &:before
    background: url('data:image/svg+xml;base64,') no-repeat center/6px 12px
.social__link--behance--black
  width: 13px
  height: 11px
  &:before
    background: url('data:image/svg+xml;base64,') no-repeat center/13px 11px

.footer__menu--06
  display inline-block
  vertical-align top
  margin-right: 65px
  &:last-child
    margin-right: 0
@media (max-width: 500px)
  .footer__menu--06:last-child
    width:100%
@media (max-width: 500px)
  .footer__menu--06
    float:left;
    width: 49%;
    margin-right: 0;
    margin-bottom: 20px

.footer__menu__item
  margin-bottom: 15px
@media (min-width: 993px)
  .footer__menu__item
    &:last-child
      margin-bottom: 0
.footer__menu__title
  font-family: Montserrat, Helvetica, sans-serif
  color: rgba(51, 52, 53, 0.3)
  font-size 11px
  letter-spacing 0.05em
  text-transform uppercase

.footer__menu__link
  position relative
  color: #333435;
  letter-spacing 0.05em
  font-size 13px
  font-weight:700
  text-decoration none
  transition opacity 0.2s ease
  &:before
    content ''
    opacity 0
    position absolute
    left 0
    right: 0
    bottom: -4px
    width: 100%
    height: 2px
    background-color: #6a6969
    transform translateY(2px) rotate(1deg)
    transition opacity 0.3s ease, transform .2s ease
  &:hover
    &:before
      opacity 1
      transform none

.footer__form
  position relative
.footer__input
  max-width 207px
  height 42px
  line-height 42px
  background-color: #fff
  border-radius 42px
  border 1px solid #ebebeb
  padding 0 40px 0 15px
  transition border-color .2s ease;
  &::-webkit-input-placeholder
    color rgba(106,105,105,.6)
    font-size 13px
    transition filter .2s ease, opacity .2s .2s ease
  &::-moz-placeholder
    color rgba(106,105,105,.6)
    font-size 13px
    transition filter .2s ease, opacity .2s .2s ease
  &::-ms-input-placeholder
    color rgba(106,105,105,.6)
    font-size 13px
    transition filter .2s ease, opacity .2s .2s ease
  &:focus
    border-color #333435
    &::-webkit-input-placeholder
      opacity 0
      filter: blur(3px)
    &::-moz-placeholder
      opacity 0
      filter: blur(3px)
    &::-ms-input-placeholder
      opacity 0
      filter: blur(3px)
@media (max-width: 500px)
  .footer__input
    max-width:100%;
    width: 100%
.footer__button
  position absolute
  right: 10px
  top: 0
  bottom: 0
  border-radius 50%
  cursor pointer
  margin auto
  width: 20px
  height: 20px
  border 1px solid rgba(51,52,53,0.66)
  background: url('data:image/svg+xml;base64,') no-repeat center / 10px auto
  transition border-color .2s ease, background-color .2s ease, transfrom .2s ease
  &:hover
    border-color #209cff
    transform translateX(3px) scale(1.1)
    background: url('data:image/svg+xml;base64,') no-repeat center / 9px 10px #209cff
.footer__message
  display none
  position absolute
  left: 0
  right: 0
  padding: 15px 0 0 0
  font-size 10px
  text-transform uppercase
  font-weight: 700
.footer__message--red
  color: #f84b29
.footer__message--green
  color #21d593
.footer__description_box--06
  padding 23px 0 25px
.footer__copyright
  font-size 10px
  font-weight: 700
  text-transform uppercase
  letter-spacing 0.05em
.footer__copyright--06
  float: right
@media (max-width: 992px)
  .footer__copyright--06
    float:left
@media (min-width: 993px)
  .footer__right_box--06
    float: right
</style>

